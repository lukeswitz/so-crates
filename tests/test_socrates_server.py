...
'''
    ...
    _FAKE_SUPPORT = {
        ...
        'interfaces': [{'name': 'en0', 'address': '10.0.0.5'},
                       {'name': 'en1', 'address': None}],
        'default_interface': 'en0', 'max_duration': 3600,
        'default_duration': 60, 'host_label': 'this Mac',
    }

    def test_capture_support_is_proxied_to_live_capture(self):
        with unittest.mock.patch('socrates.live_capture.capture_support',
                                 return_value=self._FAKE_SUPPORT):
            status, body = self._get('/api/capture-support')
        self.assertEqual(status, 200)
        self.assertEqual(json.loads(body), self._FAKE_SUPPORT)

    def test_capture_rejected_when_unsupported(self):
        """An unsupported host must fail with the specific reason rather
        than starting a job that can only die inside tcpdump."""
        unsupported = dict(self._FAKE_SUPPORT, supported=False,
                           reason='No packet-capture permission.')
        with unittest.mock.patch('socrates.live_capture.capture_support',
                                 return_value=unsupported), \
             unittest.mock.patch('socrates.live_capture.run_capture') as run:
            status, body = self._post('/api/capture', {'duration': 10})
        self.assertEqual(status, 400)
        self.assertIn('permission', json.loads(body)['error'])
        run.assert_not_called()

    def test_capture_rejects_unknown_interface(self):
        """The interface must be one live_capture actually enumerated - a
        client-supplied name never reaches the tcpdump argv on trust."""
        with unittest.mock.patch('socrates.live_capture.capture_support',
                                 return_value=self._FAKE_SUPPORT), \
             unittest.mock.patch('socrates.live_capture.run_capture') as run:
            status, body = self._post('/api/capture',
                                      {'duration': 10, 'interface': 'en0; id'})
        self.assertEqual(status, 400)
        self.assertIn('unknown interface', json.loads(body)['error'])
        run.assert_not_called()

    def test_capture_rejects_out_of_range_and_non_integer_durations(self):
        with unittest.mock.patch('socrates.live_capture.capture_support',
                                 return_value=self._FAKE_SUPPORT), \
             unittest.mock.patch('socrates.live_capture.run_capture') as run:
            for duration in (0, -5, config.CAPTURE_MAX_DURATION + 1, '60', 1.5, True, None):
                status, body = self._post('/api/capture', {'duration': duration})
                self.assertEqual(status, 400, f'duration={duration!r} must be rejected')
        run.assert_not_called()

    def test_capture_starts_and_status_reports_progress(self):
        def fake_run(dest_path, interface, duration, on_progress=print,
                     on_tick=None, should_stop=None):
            on_progress('Capturing on en0 for 5 seconds...')
            if on_tick:
                on_tick(1, 4, 2048)
            return True, 7

        with unittest.mock.patch('socrates.threading.Thread', new=SyncThread), \
             unittest.mock.patch('socrates.live_capture.capture_support',
                                 return_value=self._FAKE_SUPPORT), \
             unittest.mock.patch('socrates.live_capture.run_capture', side_effect=fake_run), \
             unittest.mock.patch('socrates._ingest_captured_pcap', return_value='a' * 32), \
             unittest.mock.patch('socrates.os.path.getsize', return_value=2048):
            status, body = self._post('/api/capture', {'duration': 5, 'interface': 'en0'})
            self.assertEqual(status, 200)
            self.assertEqual(json.loads(body),
                             {'status': 'started', 'interface': 'en0', 'duration': 5})

            status2, body2 = self._get('/api/capture-status')

        self.assertEqual(status2, 200)
        state = json.loads(body2)
        self.assertFalse(state['running'])
        self.assertTrue(state['done'])
        self.assertIsNone(state['error'])
        self.assertEqual(state['packets'], 7)
        self.assertEqual(state['md5'], 'a' * 32)
        self.assertIn('Capturing on en0 for 5 seconds...', state['lines'])

    def test_capture_status_never_leaks_the_stop_flag(self):
        """stop_requested is internal job bookkeeping, not client state."""
        status, body = self._get('/api/capture-status')
        self.assertEqual(status, 200)
        self.assertNotIn('stop_requested', json.loads(body))

    def test_capture_defaults_to_the_enumerated_interface(self):
        captured = {}
        ...
        with unittest.mock.patch('socrates.threading.Thread', new=SyncThread), \
             unittest.mock.patch('socrates.live_capture.capture_support',
                                 return_value=self._FAKE_SUPPORT), \
             unittest.mock.patch('socrates.live_capture.run_capture', side_effect=fake_run):
            status, _ = self._post('/api/capture', {'duration': 5})
        self.assertEqual(status, 200)
        self.assertEqual(captured['interface'], 'en0')

    def test_second_capture_blocked_while_one_is_running(self):
        release = threading.Event()
        entered = threading.Event()

        def blocking_run(dest_path, interface, duration, on_progress=print,
                         on_tick=None, should_stop=None):
            entered.set()
            release.wait(timeout=5)
            return False, None
        ...
        with unittest.mock.patch('socrates.live_capture.capture_support',
                                 return_value=self._FAKE_SUPPORT), \
             unittest.mock.patch('socrates.live_capture.run_capture', side_effect=blocking_run):
            status, _ = self._post('/api/capture', {'duration': 30, 'interface': 'en0'})
            self.assertEqual(status, 200)
            self.assertTrue(entered.wait(timeout=5), 'capture job must have started')

            status2, body2 = self._post('/api/capture', {'duration': 30, 'interface': 'en0'})
            self.assertEqual(status2, 409)
            self.assertIn('already in progress', json.loads(body2)['error'])

            status3, body3 = self._post('/api/capture-stop', {})
            self.assertEqual(status3, 200)
            self.assertEqual(json.loads(body3), {'status': 'stopping'})

            release.set()
            for _ in range(50):
                _, status_body = self._get('/api/capture-status')
                if json.loads(status_body)['done']:
                    break
                time.sleep(0.1)
            else:
                self.fail('capture job did not finish after release')

    def test_capture_stop_with_nothing_running_returns_409(self):
        status, body = self._post('/api/capture-stop', {})
        self.assertEqual(status, 409)
        self.assertIn('No capture is in progress', json.loads(body)['error'])
