...
- **Zip safety:** `validate_zip_extraction()` checks both zip-slip (every extracted path stays within the target directory) and zip-bomb (per-member and total decompressed-size limits). A ZIP member with a spoofed declared size (small metadata, large real decompressed content) is not a bypass of this check - CPython's `zipfile` module bounds every read to the declared size internally and then validates CRC32 against the original checksum, so a size lie surfaces as `zipfile.BadZipFile` on its own, independent of this check. See the docstring on `validate_zip_extraction()` in `validators.py` for how this was verified, before re-investigating it again.
- **Error handling:** Generic "Internal server error" - no stack traces or internal paths leaked
- **Response headers:** `_add_security_headers()` sends `X-Frame-Options: DENY`, `X-Content-Type-Options: nosniff`, and `Content-Security-Policy: default-src 'self'; ...` on every response - `'unsafe-inline'` is allowed for `script-src`/`style-src` since the UI relies on inline `onclick`/`style` attributes by convention (see `AGENTS.md`'s frontend conventions)
- **No CORS:** No `Access-Control-Allow-Origin` header is ever sent - cross-origin access isn't allowed, not even a wildcard
