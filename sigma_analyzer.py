...
# Shipped in-repo, not downloaded: Zircolite-Rules-v2 publishes no macOS ruleset.
BUNDLED_SIGMA_RULESETS = {
    'macos': os.path.join(os.path.dirname(os.path.abspath(__file__)), 'rules', 'macos.json'),
}
...
def setup_sigma_rules(data_dir=None, on_progress=print, network_allowed=True, force=False):
    ...
    _install_bundled_sigma_rules(rules_dir, result, on_progress)
    return result


def _install_bundled_sigma_rules(rules_dir, result, on_progress):
    """Copy in-repo rulesets into rules_dir, refreshing an out-of-date copy.

    No network is involved, so this runs regardless of network_allowed: the
    source is a file that shipped with this checkout, and a newer one means
    the checkout was updated.
    """
    for name, source in BUNDLED_SIGMA_RULESETS.items():
        if not os.path.isfile(source):
            continue
        dest = os.path.join(rules_dir, f'{name}.json')
        try:
            if not os.path.isfile(dest) or os.path.getmtime(source) > os.path.getmtime(dest):
                shutil.copyfile(source, dest)
            result[name] = dest
        except OSError as e:
            on_progress(f'Warning: could not install bundled Sigma rules ({name}): {e}')
...
def _looks_like_macos_events(text):
    """True if text is scripts/mac-collect-logs.sh output.

    Identified by its own EventType marker plus a mapped Sigma field, so a
    Windows Sysmon JSON export carrying Image/CommandLine can't match: only
    this collector emits EventType values naming Endpoint Security events.
    """
    if not re.search(r'"EventType"\s*:\s*"(exec|create|open|unlink|rename)"', text):
        return False
    return any(f'"{field}"' in text for field in ('Image', 'CommandLine', 'TargetFilename'))


def _detect_log_type(log_path):
    """Auto-detect whether a log file is Windows, Linux or macOS flavored.

    Returns 'windows', 'linux', 'macos', or None.
    """
    ...
    # Checked before Windows/Linux scoring: macOS events use Sigma's own
    # field names, which overlap both indicator lists.
    if _looks_like_macos_events(text):
        return 'macos'
...
def run_sigma_pipeline(dir_path, log_path, data_dir=None):
    ...
    if detected == 'linux' and rules.get('linux'):
        ruleset = rules['linux']
    elif detected == 'macos' and rules.get('macos'):
        ruleset = rules['macos']
